...
#pragma region Members
		...
		std::vector<std::string>& SavedMachineState::Neighbors()
		{
			return m_neighbors_;
		}
		/// <summary>
		...
		/// </summary>
		/// <param name="unique_key">unique name for the stateFlags</param>
		...
		SavedMachineState::SavedMachineState(std::string unique_key, StateFlags flags, std::string name, std::string description, std::string neighbors)
		{
			...
			std::stringstream string_stream(neighbors);
...
#pragma region Helper Functions
		...
		bool SavedMachineState::IsMyName(std::string name_to_test)
		{
		...
		void SavedMachineState::ReadXml(xml_node<> * room_node)
		{
			...
			std::stringstream neighbors_stream(room_node->first_node("Neighbors")->value());
			std::string sub_string;
...
#pragma region State Overrides
		std::string SavedMachineState::GetName()
		{
			return m_name_;
		}
